...
n2 <- read.dta("natsal_2000_for_archive.dta")
n2 <- n2[(n2$c_result %in% c(0,1)),] # keep only respondents with a chlamydia test result

n2$newnocon <- NA # number of new partners without a condom (including existing partnerships, where condom not used for the first time)
n2$newnocon[n2$nonocon == 0] <- 0 # no partners without a condom => no new partners without a condom
n2$newnocon[n2$het1yr == n2$nonocon] <- n2$hetnonew[n2$het1yr == n2$nonocon] # all partners without condom => all new partners were without a condom (unless stopped using a condom with someone during this year)
n2$newnocon[is.na(n2$newnocon)] <- n2$nonocon[is.na(n2$newnocon)] # otherwise, assume all partners without a condom except 1 were new
...
n2_test <- n2[(n2$het1yr < 900) & 
                (n2$nonocon < 900) & 
                (n2$hetnonew < 900),]

table(n2_test$nonocon[is.na(n2_test$newnocon)],
      n2_test$hetnonew[is.na(n2_test$newnocon)]
      )
table(n2_test$hetnonew)

table(n2_test$het1yr==n2_test$nonocon)
table(n2_test$het1yr,n2_test$nonocon)

table(n2_test$het1yr==n2_test$hetnonew)
table(n2_test$het1yr, n2_test$hetnonew)

table((n2_test$nonocon==0) | 
        (n2_test$hetnonew==0) |
        (n2_test$het1yr==n2_test$nonocon) |
        (n2_test$het1yr==n2_test$hetnonew)
      )
...
table(n2$nonocon, n2$hetnonew)	# no. of vaginal/het.anal partners in last year without a condom
table(n2$nocondom, n2$hetnonew)	# no. of het./hom. sex partners without a condom, last year
table(n2_test$het1yr - n2_test$nonocon - n2_test$hetnonew)
table(n2_test$het1yr - n2_test$nonocon - n2_test$hetnonew, n2_test$het1yr)
table(n2_test$het1yr - n2_test$nonocon - n2_test$hetnonew, n2_test$hetnonew)

table(n2$nonocon[n2$hetnonew == 0], n2$het1yr[n2$hetnonew == 0])
...
for(i in 1:(nrow(ntab_m))){
  ...
  ntab_m$prop_wt[i] <- svymean(
    ~ (newnocon) == as.numeric(as.character(ntab_m$n[i])),
...
dt <- list(
  ...
  # survey weights
  wt_m = n2$urine_wt[(n2$rsex == 'male') & (n2$het == 'het. sex. intercourse aged 13+') & (n2$het1yr %in% 1:901) & (n2$nonocon %in% 1:900) & (n2$newnocon < 900) & (n2$hetnonew != 999)],
  wt_f = n2$urine_wt[(n2$rsex == 'female') & (n2$het == 'het. sex. intercourse aged 13+') & (n2$het1yr %in% 1:901) & (n2$nonocon %in% 1:900) & (n2$newnocon < 900) & (n2$hetnonew != 999)],
  
  # infection status 
  infect_m = n2$c_result[(n2$rsex == 'male') & (n2$het == 'het. sex. intercourse aged 13+') & (n2$het1yr %in% 1:901) & (n2$nonocon %in% 1:900) & (n2$newnocon < 900) & (n2$hetnonew != 999)],
  infect_f = n2$c_result[(n2$rsex == 'female') & (n2$het == 'het. sex. intercourse aged 13+') & (n2$het1yr %in% 1:901) & (n2$nonocon %in% 1:900) & (n2$newnocon < 900) & (n2$hetnonew != 999)],
...
#save(fit_all, file= 'transmission_probability_reparam_natsal2_nonocon_1000.RData')
...
plot(rep(h1$breaks, each=2), c(0, rep(h1$density, each=2), 0), 
     ...
     xlab="Transmission probability per partnership (%)", ylab='Density', main="Natsal-2 opposite-sex partnerships"
     )
lines(rep(h2$breaks, each=2), c(0, rep(h2$density, each=2), 0), col='#00A08A', lwd=3, lty=3 )
