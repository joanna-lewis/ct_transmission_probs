...
functions {
  ...
  real[] prev_int(real t,
                 ...
                 int[] x_i) {
        real dydt[1];
        dydt[1] = exp(-theta[2]*t) * t^theta[1] / (t + (theta[4]/theta[3] ));
        return dydt;
  }

  // integrator to help with calculating per-partnership prevalence
  real[] ppp_int(real t,
                 real[] y,
                 real[] theta, // (alpha, beta, A, lambda)
                 real[] x_r,
                 int[] x_i) {
        real dydt[1];
        dydt[1] = exp(-theta[2]*t) * t^(theta[1] + 1.0) / (t + (theta[4]/theta[3] ));
        return dydt;
  }
...
parameters {
  ...
  // hyperparameters
  real<lower=0> mu_nb[2]; // parameterises gamma distribution (male, female)
  ...
  real<lower=0,upper=1> A[2]; // force of infection per partner (men, women): per-partnership prevalence in opposite sex, multiplied by transmission probability
...
transformed parameters {
  ...
  real<lower=0,upper=1> pi_m[max(n_m) + 1]; // prevalence in men reporting each number
  real<lower=0,upper=1> pi_f[max(n_f) + 1]; // prevalence in women reporting each number
  
  real theta[2,4]; // parameters for ODE solver  
  real init[1]; // initial condition for ODE solver
  
  real<lower=0,upper=1> ppp[2]; // per-partnership prevalence in partnerships offered by (men, women)
  
  alpha[1] = mu_nb[1] * beta[1];
  alpha[2] = mu_nb[2] * beta[2];
  ...
  ppp[1] = beta[1]^(alpha[1]+1) * integrate_ode_rk45(ppp_int, init, 0.0, s_max, theta[1], x_r, x_i)[1,1] / (alpha[1]*tgamma(alpha[1]));
  if(is_nan(ppp[1]))
    ppp[1] = -999;

  ppp[2] = beta[2]^(alpha[2]+1) * integrate_ode_rk45(ppp_int, init, 0.0, s_max, theta[2], x_r, x_i)[1,1] / (alpha[2]*tgamma(alpha[2]));
  if(is_nan(ppp[2]))
    ppp[2] = -999;
...
model {
  ...
  A[1] ~ exponential(0.001); //uniform(0,1); //ppp[2]); // force of infection / sigma
  A[2] ~ exponential(0.001); //uniform(0,1); //ppp[1]); 
...
generated quantities {
  ...
  real rho_mf; // male-to-female transmission probability
  real rho_fm; // female-to-male transmission probability
  ...
  real theta_gq[2,4]; // parameters for ODE solver  
  real init_gq[1]; // initial condition for ODE solver
  real<lower=0, upper=1> pi_overall[2]; // overall prevalence in men and women
  ...
  theta_gq[1,1] = alpha[1];
  theta_gq[1,2] = beta[1];
  theta_gq[1,3] = A[1];
  theta_gq[1,4] = lambda_slow[1];
  theta_gq[2,1] = alpha[2];
  theta_gq[2,2] = beta[2];
  theta_gq[2,3] = A[2];
  theta_gq[2,4] = lambda_slow[2];
  init_gq[1] = 0.0;

  pi_overall[1] = (beta[1]^alpha[1]) * integrate_ode_rk45(prev_int, init_gq, 0.0, s_max, theta_gq[1], x_r, x_i)[1,1] / tgamma(alpha[1]);
  pi_overall[2] = (beta[2]^alpha[2]) * integrate_ode_rk45(prev_int, init_gq, 0.0, s_max, theta_gq[2], x_r, x_i)[1,1] / tgamma(alpha[2]);
